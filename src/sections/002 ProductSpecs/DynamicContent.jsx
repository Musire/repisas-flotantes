...
export default function DynamicContent ({ variant }) {
    ...
    return (
        <TabsContent 
            value={variant} 
            className=" w-full h-fit flex flex-col md:flex-row items-center space-y-12 min-h-[40dvh]  lg:justify-center p-4"
        >
            ...
            <button type="button" className="font-bold flex items-center space-x-2 ">
                <Caption className="text-lg cursor-pointer" onClick={toggleDrawer} >Ver Detalles</Caption>
                <ChevronRight />    
            </button>
            <ul className="w-full flex flex-row overflow-scroll scrollbar-none gap-6 md:max-lg:spaced min-h-[40dvh] h-fit lg:h-full lg:flex-col lg:w-1/3 ">
                {bundleData?.map(card => {
